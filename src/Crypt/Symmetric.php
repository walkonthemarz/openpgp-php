...
use Exception;
use OpenPGP\Packets\ModificationDetectionCode;
use IteratorAggregate;
use OpenPGP\Packets\IntegrityProtectedData;
use OpenPGP\Packets\PublicKey;
use OpenPGP\Packets\AsymmetricSessionKey;
use OpenPGP\Pgp;
use OpenPGP\S2k;
use OpenPGP\Packets\SymmetricSessionKey;
use OpenPGP\Message;
use OpenPGP\Packets\SecretKey;
use OpenPGP\Packets\EncryptedData;
...
class Symmetric {
    public static function encrypt($passphrases_and_keys, $message, $symmetric_algorithm=9) {
        ...
        $mdc = new ModificationDetectionCode(hash('sha1', $to_encrypt . "\xD3\x14", true));
        $to_encrypt .= $mdc->to_bytes();
        $encrypted = array(new IntegrityProtectedData($cipher->encrypt($to_encrypt)));
        ...
        foreach($passphrases_and_keys as $pass) {
            if($pass instanceof PublicKey) {
                ...
                $rsa->setEncryptionMode(RSA::CRYPT_RSA_ENCRYPTION_PKCS1);
                $esk = $rsa->encrypt(chr($symmetric_algorithm) . $key . pack('n', self::checksum($key)));
                $esk = pack('n', Pgp::bitlength($esk)) . $esk;
                array_unshift($encrypted, new AsymmetricSessionKey($pass->algorithm, $pass->fingerprint(), $esk));
            } else if(is_string($pass)) {
                $s2k = new S2k(Random::string(8));
                $cipher->setKey($s2k->make_key($pass, $key_bytes));
                $esk = $cipher->encrypt(chr($symmetric_algorithm) . $key);
                array_unshift($encrypted, new SymmetricSessionKey($s2k, $esk, $symmetric_algorithm));
            }
        }
    ...
    public static function decryptSymmetric($pass, $m) {
        ...
        foreach($m as $p) {
            if($p instanceof SymmetricSessionKey) {
    ...
    public static function encryptSecretKey($pass, $packet, $symmetric_algorithm=9) {
        ...
        $material = '';
        foreach(SecretKey::$secret_key_fields[$packet->algorithm] as $field) {
            $f = $packet->key[$field];
            $material .= pack('n', Pgp::bitlength($f)) . $f;
            unset($packet->key[$field]);
        }
        $material .= hash('sha1', $material, true);

        $iv = Random::string($key_block_bytes);
        if(!$packet->s2k) $packet->s2k = new S2k(Random::string(8));
    ...
    public static function decryptPacket($epacket, $symmetric_algorithm, $key) {
        ...
        if($epacket instanceof IntegrityProtectedData) {
    ...
    public static function getCipher($algo) {
        ...
        switch($algo) {
            ...
            case 2:
                ...
                break;
            case 3:
                if(class_exists('OpenPGP\\Crypt\\OpenSSLWrapper')) {
    ...
    public static function getEncryptedData($m) {
        foreach($m as $p) {
            if($p instanceof EncryptedData) return $p;
        }
