...
use OpenPGP\Packets\Signatures\SignatureCreationTime;
use OpenPGP\Message;
use Exception;
...
class Signature extends Packet
{
    ...
    function __construct($data = null, $key_algorithm = null, $hash_algorithm = null)
    {
        ...
        if (is_string($this->key_algorithm = $key_algorithm)) {
            $this->key_algorithm = array_search($this->key_algorithm, PublicKey::$algorithms);
        }
        if ($data) { // If we have any data, set up the creation time
            $this->hashed_subpackets = [new SignatureCreationTime(time())];
        }
        if ($data instanceof LiteralData) {
            $this->signature_type = ($data->format == 'b') ? 0x00 : 0x01;
            $data->normalize();
            $data = $data->data;
        } elseif ($data instanceof Message && $data[0] instanceof PublicKey) {
        ...
        switch ($this->version = ord($this->read_byte())) {
            ...
            case 3:
                ...
                $this->unhashed_subpackets = [
                    new SignatureCreationTime($creation_time),
                    new Issuer($keyidHex),
                ];
        ...
        switch ($this->version) {
            ...
            case 3:
                ...
                foreach ($this->data as $mpi) {
                    $body .= pack('n', Pgp::bitlength($mpi)) . $mpi;
                }
    ...
    {
        return PublicKey::$algorithms[$this->key_algorithm];
    }
    ...
    static function class_for($tag)
    {
        if (!isset(self::$subpacket_types[$tag])) {
            return 'OpenPGP\\Packets\\Signatures\\Sub';
        }

        return 'OpenPGP\\Packets\\Signatures\\' . self::$subpacket_types[$tag];
    }
