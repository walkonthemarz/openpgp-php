...
class S2k
{
    ...
    static function parse(&$input)
    {
        ...
        switch ($s2k->type = ord($input[0])) {
            ...
            case 3:
                $s2k->hash_algorithm = ord($input[1]);
                $s2k->salt           = substr($input, 2, 8);
                $s2k->count          = Pgp::decode_s2k_count(ord($input[10]));
    ...
    function to_bytes()
    {
        ...
        switch ($this->type) {
            ...
            case 3:
                ...
                $bytes .= chr($this->hash_algorithm);
                $bytes .= $this->salt;
                $bytes .= chr(Pgp::encode_s2k_count($this->count));
                break;
        }
    ...
    function raw_hash($s)
    {
        return hash(strtolower(Signature::$hash_algorithms[$this->hash_algorithm]), $s, true);
    }
